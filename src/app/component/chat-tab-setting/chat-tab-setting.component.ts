...
export class ChatTabSettingComponent implements OnInit, OnDestroy {
  ...
  get isDeleted(): boolean {
    return this.selectedTab
      ? ObjectStore.instance.get(this.selectedTab.identifier) == null
      : false
  }
  ...
  ngOnInit() {
    ...
    EventSystem.register(this).on('DELETE_GAME_OBJECT', 1000, (event) => {
      if (
        !this.selectedTab ||
        event.data.identifier !== this.selectedTab.identifier
      )
        return
  ...
  restore() {
    if (this.selectedTab && this.selectedTabXml) {
      let restoreTable = <ChatTab>(
        ObjectSerializer.instance.parseXml(this.selectedTabXml)
      )
