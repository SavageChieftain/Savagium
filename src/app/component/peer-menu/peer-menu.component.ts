...
export class PeerMenuComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  connectPeer() {
    this.help = ''
    const context = PeerContext.create(this.targetPeerId)
    if (!context.isRoom) {
      ...
      if (Network.peerContexts.length) {
        this.help =
          '入力されたIDはルーム用のIDのようですが、ルーム用IDと通常のIDを混在させることはできません。プライベート接続を切ってください。（※ページリロードで切断ができます）'
        return
      }
      ...
      const dummy = {}
      EventSystem.register(dummy).on('OPEN_NETWORK', (event) => {
        ObjectStore.instance.clearDeleteHistory()
        Network.connect(this.targetPeerId)
  ...
  async connectPeerHistory() {
    ...
    for (const peer of this.appConfigService.peerHistory) {
      const context = PeerContext.create(peer)
      if (context.isRoom) {
    ...
    listener.on('OPEN_NETWORK', (event) => {
      ...
      ObjectStore.instance.clearDeleteHistory()
      for (const context of conectPeers) {
