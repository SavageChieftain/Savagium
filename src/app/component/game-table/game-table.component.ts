...
import {
  TableTouchGesture,
  TableTouchGestureEvent,
} from './table-touch-gesture'
...
export class GameTableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  initializeTableTouchGesture() {
    this.gesture = new TableTouchGesture(
  ...
  onTableTouchTransform(
    ...
    if (
      event === TableTouchGestureEvent.PAN &&
      (!this.isTransformMode || this.input.isGrabbing)
    )
      return
