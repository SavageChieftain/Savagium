...
export class GameTableComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  get tableImage(): ImageFile {
    const file: ImageFile = ImageStorage.instance.get(this.currentTable.imageIdentifier)
    return file || ImageFile.Empty
  }

  get backgroundImage(): ImageFile {
    const file: ImageFile = ImageStorage.instance.get(this.currentTable.backgroundImageIdentifier)
    return file || ImageFile.Empty
  }
  ...
  private viewPotisonX = 100

  private viewPotisonY = 0

  private viewPotisonZ = 0

  private viewRotateX = 50

  private viewRotateY = 0

  private viewRotateZ = 10

  private buttonCode = 0
  ...
  onTableTouchTransform(
    ...
    if (event === TableTouchGestureEvent.PAN && (!this.isTransformMode || this.input.isGrabbing))
      return
    ...
    transformX *= scale
    transformY *= scale
    if (rotateX + this.viewRotateX > 80) rotateX += 80 - (rotateX + this.viewRotateX)
  ...
  onInputMove(e: any) {
    ...
    const { x } = this.input.pointer
    const { y } = this.input.pointer
