...
export class TooltipDirective implements OnInit, AfterViewInit, OnDestroy {
  ...
  private onMouseDown(e: any) {
    ...
    if (
      !this.tooltipComponentRef.location.nativeElement.contains(e.target) &&
  ...
  private startOpenTimer() {
    const { pointerX } = this.pointerDeviceService
    const { pointerY } = this.pointerDeviceService

    this.openTooltipTimer = setTimeout(() => {
      this.openTooltipTimer = null
      const magnitude =
        (pointerX - this.pointerDeviceService.pointerX) ** 2 +
        (pointerY - this.pointerDeviceService.pointerY) ** 2
      if (magnitude > 4) {
        this.startOpenTimer()
      } else {
  ...
  private open() {
    ...
    const { injector } = parentViewContainerRef
