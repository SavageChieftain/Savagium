...
export class DraggableDirective implements AfterViewInit, OnDestroy {
  @Input('draggable.disable') isDisable = false

  @Input('draggable.bounds') boundsSelector = 'body'

  @Input('draggable.handle') handleSelector = ''

  @Input('draggable.unhandle') unhandleSelector = 'input,textarea,button,select,option,span'

  @Input('draggable.stack') stackSelector = ''

  @Input('draggable.opacity') opacity = 0.7
  ...
  private onInputStart(e: MouseEvent | TouchEvent) {
    ...
    const isScrollable =
      (e as TouchEvent).touches != null ? this.isScrollableElement(e.target as HTMLElement) : false
  ...
  private isScrollableElement(target: HTMLElement) {
    ...
    while (node && boundsElm !== node && this.elementRef.nativeElement !== node) {
      ...
      if (overflowType.indexOf(css.overflowY) >= 0 && node.offsetHeight < node.scrollHeight)
        return true
      if (positionType.indexOf(css.position) >= 0) return false
      node = node.parentElement
    }
    return false
  }

  private calcCorrectionPosition(
    diff: PointerCoordinate = { x: 0, y: 0, z: 0 },
  ): PointerCoordinate {
  ...
  private calcElementPosition(target: HTMLElement): PointerCoordinate {
    ...
    return {
      x: CSSNumber.relation(
        css.left,
        target.parentElement.offsetWidth,
        target.parentElement.offsetWidth * 0.5,
      ),
      y: CSSNumber.relation(
        css.top,
  ...
  private setForeground() {
    ...
    const stacks = this.elementRef.nativeElement.ownerDocument.querySelectorAll<HTMLElement>(
      this.stackSelector,
    )
    let topZindex = 0
