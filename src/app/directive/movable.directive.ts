...
export class MovableDirective implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    EventSystem.register(this).on('UPDATE_GAME_OBJECT', -1000, (event) => {
      if (
  ...
  onInputMove(e: MouseEvent | TouchEvent) {
    ...
    this.posX =
      this.pointer3d.x +
      this.pointerOffset3d.x * this.ratio +
      -(this.width / 2) * (1.0 - this.ratio)
    this.posY =
      this.pointer3d.y +
      this.pointerOffset3d.y * this.ratio +
      -(this.height / 2) * (1.0 - this.ratio)
  ...
  private calcLocalCoordinate(
    ...
    if (target.contains(this.tabletopService.dragAreaElement)) {
      coordinate = PointerDeviceService.convertToLocal(
        coordinate,
        this.tabletopService.dragAreaElement,
      )
      coordinate.z = 0
    } else {
  ...
  private shouldTransition(object: TabletopObject): boolean {
    return (
      object.location.x !== this.posX ||
      object.location.y !== this.posY ||
      object.posZ !== this.posZ
    )
  }
